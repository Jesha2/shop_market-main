require('dotenv').config()//imports the dotenv and invokes its config() so that we can access our JWT_SECRET var from .env file. The dotenv package is used to load environment variables from a .env file into Node.js application. It allows to store configuration data, such as secrets, without hardcoding them into your code.
...
const {JWT_SECRET} = process.env//destructure JWT_SECRET env var from the .env file.This variable is typically used as the secret key for signing and verifying JWTs.

//The following code exports an object with an isAuthenticated method, which is an Express.js middleware function
module.exports = {
    isAuthenticated: (req, res, next) => {//this method is called in server.js before calling any of the post router function to check if user has authorization for accessing the posts section to add, edit/delete. if its not post function and just viewing website, no need to authenticate
...
const { JWT_SECRET } = process.env: Here, the SECRET environment variable is being destructured from the process.env object. This assumes that you have defined a SECRET variable in your .env file. This variable is typically used as the secret key for signing and verifying JWTs.
